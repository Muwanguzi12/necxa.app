...
router.post('/:id/like', async (req: Request, res) => {
  ...
  const idempotencyKey = (req.headers['idempotency-key'] || req.headers['Idempotency-Key'] || req.headers['Idempotency-key']) as string | undefined;
  try {
    const db = req.app.locals.db;
    const result = await likePost(db, id, user.sub || user.id || 'unknown', idempotencyKey);
    ...
    return res.status(200).json({ postId: id, meta: result });
  } catch (err) {
...
router.post('/:id/comment', async (req: Request, res) => {
  ...
  try {
    const db = req.app.locals.db;
    const comment = await commentPost(db, id, user.sub || user.id || 'unknown', text, idempotencyKey);
    if (req.app.locals.kafkaProducer) {
    ...
    // if duplicate idempotency key was used we returned existing comment; surfacing its id
    if (err && String(err).includes('duplicate key')) {
      const db = req.app.locals.db;
      const existing = await db.collection('comments').findOne({ idempotencyKey });
      if (existing) return res.status(200).json({ postId: id, comment: { id: existing._id.toString(), text: existing.text } });
    }
